...
\begin{TASK}[float=h
  ...
  officerApprove invoiceDate today confirmed >>= \ approved.|\label{fig:taxman-officer-approve}|
  let subsidyAmount = if approved
    then min 600 (invoiceAmount / 10) else 0 in
  edit <<subsidyAmount, approved, confirmed, invoiceDate, today>>|\label{fig:taxman-result}|
\end{TASK}

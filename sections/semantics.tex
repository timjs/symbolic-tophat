...
The \refrule{E-Edit} rule evaluates

%explain what the overline notation means (using list comprehensions?)


%explain why we can have multiple results from one rule



%wat betekend dit resultaat intuitief?
...
\begin{function}
  \signature{\mathit{simulate} : \Task \times [\mathrm{Inputs}] \times \mathrm{State} \times \mathrm{Predicate} \rightarrow [(\Task,[\mathrm{Inputs}],\mathrm{State},\mathrm{Predicate})]} \\
  \mathit{simulate}\ t\ I\ \sigma \ \phi  = \bigcup
  \{\mathit{simulate'}\ \True\ t\ I\ \phi\ r\mid r \in \  (t,\sigma\drive{})\}\\
                \mathit{where}\ \mathit{simulate'}\ go\ I\ t\ \phi\ t'\ i\ \sigma'\ \phi' =\\
                        \left \{\begin{array}{ll}
                          \emptyset & \neg \text{SAT } (\phi'\land\phi)\\
                  \{(t',I\oplus[i],\sigma',\phi\land\phi')\} & \Value(t',\sigma') = v \land \text{SAT } (\phi'\land\phi)\\
                        \mathit{simulate}\ t'\ (I\oplus[i])\ \sigma'\ (\phi\land\phi') & (t' \neq t) \land \Value(t',\sigma') = \bot\\
                  \bigcup \{\mathit{simulate'}\ \False\ t'\ (I\oplus[i])\ (\phi'\land\phi)\ r\mid r \in \  (t',\sigma'\drive{})\} & go \land t' = t  \land \Value(t',\sigma') = \bot\\
                  \emptyset           & \neg go \land t' = t  \land \Value(t',\sigma') = \bot
                                  \end{array}\right.\\
\end{function}
...
\label{fig:simulate}
\end{figure*}
\fixme{arguments are in the wrong order, drive gives task, state and then input}

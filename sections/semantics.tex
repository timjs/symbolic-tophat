...
\end{itemize}
\todo{we allow for one step lookahead. Explain}\\
...
%   \mathit{simulate}\ t\ I\ \sigma \ \phi  = \mathit{concat} \ (\mathit{map} \ (\mathit{simulate'}\ \True\ t\ I\ \phi) (t,\sigma \drive{}))\\
%                 \mathit{where}\ \mathit{simulate'}\ go\ I\ t\ \phi\ t'\ i\ \sigma'\ \phi' =\\
%                         \begin{array}{ll}
...
%                   \mathit{concat} \ (\mathit{map} \ (\mathit{simulate'}\ \False\ t'\ (I\oplus[i])\ (\phi'\land\phi))\ (t',\sigma'\drive{}))           & go \land t' = t  \land \Value(t',\sigma') = \bot\\
...
%       \begin{array}{ll}
%                 \mid \neg \text{SAT } (\phi'\land\phi) & = [\ ]\\
%                 \mid \Value(t',\sigma') = v \land \text{SAT } (\phi'\land\phi) & = [(t',I +\!\!+ [i],\sigma',\phi\land\phi')]\\
%                 \mid (t' \neq t) \land \Value(t',\sigma')=\bot & =\mathit{simulate}\ t'\ (I +\!\!+ [i])\ \sigma'\ (\phi\land\phi')\\
%                 \mid go \land t' = t  \land \Value(t',\sigma') = \bot &= \mathit{concat} \ (\mathit{map} \ (\mathit{simulate'}\ \False\ t'\ (I +\!\!+ [i])\ (\phi'\land\phi))\ (t',\sigma'\drive{}))\\
%                 \mid \neg go \land t' = t  \land \Value(t',\sigma') = \bot & = [\ ]
%                                   \end{array}\\
% \end{function}

\hspace{-2cm}\begin{function}
  \signature{\mathit{simulate} : \Task \times \mathrm{State} \times [\mathrm{Inputs}]  \times \mathrm{Predicate} \rightarrow \powerset((\Task,[\mathrm{Inputs}],\mathrm{State},\mathrm{Predicate}))} \\
  \mathit{simulate}\ t\ \sigma \ I\  \phi  = \bigcup \{\mathit{simulate'}\ \True\ t\ I\ \phi\ t'\ \sigma'\ i\ \phi' \mid (t',\sigma',i,\phi') \in \  (t,\sigma\drive{})\}\\
  \signature{\mathit{simulate'} : \Bool \times \Task \times [\mathrm{Inputs}]\times \mathrm{Predicate} \times \Task \times \mathrm{State}\times \mathrm{Input} \times \mathrm{Predicate} \rightarrow \powerset((\Task,[\mathrm{Inputs}],\mathrm{State},\mathrm{Predicate}))}\\
  \mathit{simulate'}\ go\ t\ I\ \phi\ t'\ i\ \sigma'\ \phi' =\\
              \left \{\begin{array}{lccccccc}
                  \emptyset                                                                                                       &\neg \text{SAT } (\phi'\land\phi) &&&&&&\\
                  \{(t',I\oplus[i],\sigma',\phi\land\phi')\}                                                                      & \text{SAT } (\phi'\land\phi)     &\land& \Value(t',\sigma') = v &&&& \\
...
\tikzstyle{level 2}=[level distance=4.5cm, sibling distance=3.5cm]
...
\end{tikzpicture}
\caption{simulation function applied to abs}
\end{figure*}
